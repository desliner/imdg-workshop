...
import com.griddynamics.workshop.imdg.util.DateCodec;
...
public class ReflectionPofSerializer implements PofSerializer {
    ...
    static {
        ...
        // Custom
        CODEC.put(String.class, new CompressingStringCodec());
        CODEC.put(Date.class, new DateCodec());
    }
    ...
    public ReflectionPofSerializer() {
        for (final Map.Entry<Class<?>, ObjectPropCodec> codecEntry : CODEC.entrySet()) {
            formats.put(codecEntry.getKey(), new PofSerializer() {
                @Override
                public void serialize(PofWriter pofwriter, Object obj) throws IOException {
                    codecEntry.getValue().writeProp(pofwriter, 0, obj, null);
                }

                @Override
                public Object deserialize(PofReader pofreader) throws IOException {
                    return codecEntry.getValue().readProp(pofreader, 0, null);
                }
            });
        }
    }
